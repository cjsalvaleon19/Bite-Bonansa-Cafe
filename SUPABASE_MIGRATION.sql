-- ============================================================================
-- Bite Bonansa Cafe - Complete Supabase Migration
...
DECLARE
  ...
  earnings_amt DECIMAL;
BEGIN
  -- Only add points when order is delivered
  IF NEW.status = 'order_delivered' AND (OLD.status IS NULL OR OLD.status != 'order_delivered') THEN
    -- Calculate earnings amount if not already set
    IF NEW.earnings_amount IS NOT NULL THEN
      earnings_amt := NEW.earnings_amount;
    ELSE
      -- Fallback: Calculate 2% or 5% based on subtotal
      IF NEW.subtotal >= 500 THEN
        earnings_amt := NEW.subtotal * 0.05; -- 5% for orders >= ₱500
      ELSE
        earnings_amt := NEW.subtotal * 0.02; -- 2% for orders < ₱500
      END IF;
    END IF;
...
-- Pricing scheme: ₱30 base fare + tiered additional fees (capped at ₱98 for 10km)
...
-- Test the delivery fee calculation functions
-- SELECT calculate_delivery_fee(500) as fee_500m;  -- Should return 30.00
-- SELECT calculate_delivery_fee(1200) as fee_1200m;  -- Should return 35.00
-- SELECT calculate_delivery_fee(5000) as fee_5000m;  -- Should return 66.00
-- SELECT calculate_delivery_fee(15000) as fee_15000m;  -- Should return 98.00 (capped)
...
-- ============================================================================
-- DONE! All tables, functions, triggers, and RLS policies have been created.
-- ============================================================================
