-- ============================================================================
-- Migration 095: Fix cash_audits table
-- ============================================================================
-- Resolves ERROR 42703 "column cashier_id does not exist" that occurs when
-- migration 094 is run but a cash_audits table already existed (e.g., from a
-- manual creation) without the cashier_id column.
--
-- Migration 094 used CREATE TABLE IF NOT EXISTS, which silently skips table
-- creation when the table exists.  The subsequent CREATE INDEX on cashier_id
-- then fails because the pre-existing table lacks that column.
--
-- This migration drops the old (incomplete) table and recreates it with the
-- correct full schema.  Because cash_audits is a new feature there is no
-- production data to preserve.
-- ============================================================================

-- Drop any previous (potentially incomplete) version of the table
DROP TABLE IF EXISTS cash_audits CASCADE;
...
CREATE INDEX idx_cash_audits_cashier_date ON cash_audits(cashier_id, audit_date DESC);
CREATE INDEX idx_cash_audits_date ON cash_audits(audit_date DESC);
...
-- Cashiers can view their own audits; admins can view all
...
-- Cashiers can insert their own audits
...
-- Cashiers can update their own unsubmitted audits only
