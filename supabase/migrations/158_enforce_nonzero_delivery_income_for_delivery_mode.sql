-- Migration 158: Enforce non-zero Delivery Income for delivery-mode orders
-- ─────────────────────────────────────────────────────────────────────────────────────────
-- Why this exists:
--   Delivery-mode orders must never post zero Delivery Income.
--   Some rows still yield zero from stored/derived fees.
--
-- Fixes:
--   1) Trigger computes effective delivery fee from:
--        delivery_fee
--        OR (total_amount - subtotal - vat_amount)
--        OR 25.00 minimum when order_mode = 'delivery'
--   2) Backfill uses the same logic so delivery-mode orders do not remain at zero.
-- ─────────────────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  v_order_mode   := LOWER(COALESCE(NEW.order_mode, ''));
  ...
  IF v_order_mode = 'delivery' AND v_delivery_fee <= 0 THEN
    v_delivery_fee := 25;
  END IF;
...
CREATE TEMP TABLE _eligible_orders_158 ON COMMIT DROP AS
SELECT
  ...
  ROUND((
    CASE
      ...
      WHEN LOWER(COALESCE(o.order_mode, '')) = 'delivery'
        THEN 25
      ELSE 0
    END
  ) * 100) / 100 AS effective_delivery_fee,
  CASE
...
DO $$ BEGIN
  RAISE NOTICE 'Migration 158: delivery-mode orders now enforce non-zero Delivery Income.';
END $$;
