-- Migration 154:
-- Ensure Admin > Journal Entries includes all completed orders, not only
-- orders finalized with status = order_delivered.
--
-- Problem:
--   The latest order journal trigger only fired for status transitions to
--   'order_delivered'. The orders schema and related triggers also recognize
--   'completed' as a terminal/completed status, so any orders finalized as
--   'completed' could miss their sales, rewards, and COGS journal entries.
--
-- Fix:
--   1. Update the journal trigger to fire for transitions to either
--      'order_delivered' or 'completed'.
--   2. Backfill missing revenue, rewards, and COGS entries for any existing
--      completed/delivered orders without duplicating existing journal lines.
...
-- Backfill missing cash / bank / credit-card revenue rows.
INSERT INTO journal_entries
  (date, description, debit_account, credit_account, amount, reference_type, reference, name)
SELECT
  COALESCE((o.completed_at)::date, (o.created_at)::date, CURRENT_DATE),
...
DO $$ BEGIN
  RAISE NOTICE 'Migration 154: completed orders now generate and backfill journal entries.';
END $$;
